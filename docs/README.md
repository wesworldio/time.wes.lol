...
1. **Similarity matcher (`find_similar.py`)**
   - Uses search images in `data/search/` to find near-matches across images and videos in `assets/`.
   - Saves matches (with timecodes for video frames) into `data/results/`.
   - Run via `make run` (installs deps via `make install` if needed).

2. **Frame capture UI (`public/index.html`)**
...
- Deploy the repo root on GitHub Pages (root or /docs). With the root layout:
  ...
  - Assets: `assets/`, Results: `data/results/`, Search images: `data/search/`.

## Usage
- Capture tool: open the hosted `index.html`, load `assets/wesjerryspringer.mov`, click matching thumbnails (from `data/results/`) to jump to that timecode, capture a crop, and move the downloaded PNG into `data/search/` to improve training data.
- Matcher: `make run` to regenerate matches into `data/results/` after adding new training images.

## Notes
- Keep `assets/`, `data/results/`, and `data/search/` in the repo so the hosted UI can read them directly.
